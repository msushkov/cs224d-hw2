...
def part_a(clf, num_to_word, verbose=True):
    ...
    # W[...] is 100 x 50, L is |V| x 50
    # get the argmax indices of this:
    dots = dot(clf.params.W[:, 50:100], clf.sparams.L.T)
    indices = dots.argsort()[:, -10:][:, ::-1] # (100 x |V|)

    for i in xrange(indices.shape[0]):
        scores = []
        words = []
        for j in xrange(indices.shape[1]):
            curr_index = indices[i, j]
            scores.append(dots[i, curr_index])
            words.append(num_to_word[curr_index])

        topscores.append(scores)
        topwords.append(words)

    neurons = [5, 7, 11, 20, 25] # change this to your chosen neurons
...
def part_b(clf, num_to_word, num_to_tag, verbose=True):
    ...
    # W is (100, 50), L is (|V|, 50)
    # U is (5, 100)
    a = np.dot(clf.sparams.L, clf.params.W[:, 50:100].T) + clf.params.b1 # (|V|, 100)
    h = tanh(a) # (|V|, 100)
    y_hat = softmax(np.dot(h, clf.params.U.T) + clf.params.b2) # (|V|, 5)
    ...
    for i in range(5):
        # take the ith column of y_hat
        probs = y_hat[:, i] # (|V|,)
        indices = probs.argsort()[-10:][::-1]
        topscores_i = [probs[j] for j in indices]
        topwords_i = [num_to_word[j] for j in indices]

        topscores.append(topscores_i)
        topwords.append(topwords_i)
...
def part_c(clf, num_to_word, num_to_tag, verbose=True):
    ...
    # W is (100, 50), L is (|V|, 50)
    # U is (5, 100)
    a = np.dot(clf.sparams.L, clf.params.W[:, :50].T) + clf.params.b1 # (|V|, 100)
    h = tanh(a) # (|V|, 100)
    y_hat = softmax(np.dot(h, clf.params.U.T) + clf.params.b2) # (|V|, 5)
